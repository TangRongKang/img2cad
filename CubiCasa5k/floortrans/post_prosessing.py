...
def extract_wall_polygon(wall, wall_points, segmentation, seg_class):
    ...
    if w_dim == 1:
        ...
        # if len(widths) == 0:
            # return None
        # 适配新版本 scipy：mode 可能是标量或数组
        mode_result = stats.mode(widths).mode
        wall_width = mode_result[0] if hasattr(mode_result, '__len__') and len(mode_result) > 0 else mode_result
        if wall_width > y2 - y1:
            wall_width = y2 - y1
